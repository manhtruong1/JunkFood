					...
					<div class="container">
						<div class="row mt-5 mb-5">
							<div class="col-12">
								...
								<div class="card mt-4">
									<h5 class="card-header">Quản Lý Order Detail</h5>
									<div class="table-responsive">
										<table class="table table-dark table-striped">
											...
											<tbody class="table-border-bottom-0">
												<c:forEach var="orderDetail" items="${lOrderDetail}">
													<tr>
														<td>${orderDetail.orderdetailid}</td>
														<td>${orderDetail.order.customer.name}</td>
														<td>${orderDetail.order.customer.phone}</td>
														<td>${orderDetail.order.customer.diachi}</td>
														<td>${orderDetail.product.name}</td>
														<td><fmt:formatNumber value="${orderDetail.price}"
																pattern="#,###"></fmt:formatNumber></td>
														<td>${orderDetail.quantity}</td>
														<td><fmt:formatNumber
																value="${orderDetail.order.amount}" pattern="#,###"></fmt:formatNumber>
														</td>
														<th><c:if test="${orderDetail.status == true}">
																<span class="text-success">${orderDetail.status == true ? "Đã Duyệt": "Chưa Duyệt"}</span>
															</c:if> <c:if test="${orderDetail.status == false}">
																<span class="text-danger">${orderDetail.status == true ? "Đã Duyệt": "Chưa Duyệt"}</span>

															</c:if></th>
														<td>
															
																 <a href="/junkfood/orderdetail/comfirm"
																	class="btn btn-warning text-white">Xác Nhận</a>
																	
														</td>
														<td><a href="" class="btn btn-danger text-white">Hủy
																Đơn</a></td>
													</tr>
												</c:forEach>
											</tbody>
